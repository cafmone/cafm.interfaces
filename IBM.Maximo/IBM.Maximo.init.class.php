...
* @var string
*/
var $actions_name = 'ibm_maximo_action';
/**
* message param
* @access public
* @var string
*/
var $message_param = 'ibm_maximo_msg';
/**
* translation
* @access public
* @var array
*/
var $lang = array(
	'description' => 'IBM Maximo Interface',
	'label' => 'IBM Maximo',
	'settings' => 'Settings',
);
/**
...
var $PROFILESDIR;
	...
	function __construct($response, $file, $user, $db) {
		...
		$this->user = $user;
		$this->db = $db;
		$this->lang = $this->user->translate($this->lang, CLASSDIR.'plugins/IBM.Maximo/lang/', 'ibm.maximo.init.ini');
	}
	...
	function menu() {
		...
		$settings = $this->file->get_ini($this->PROFILESDIR.'/IBM.Maximo.ini');
		...
		if($this->user->is_admin($user) || in_array($user, $users)) {
			...
			$t = $response->html->template($this->tpldir.'IBM.Maximo.menu.html');
			$t->add($links, 'links');
